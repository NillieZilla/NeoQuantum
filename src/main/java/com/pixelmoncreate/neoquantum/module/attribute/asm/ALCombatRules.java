package com.pixelmoncreate.neoquantum.module.attribute.asm;
...
public class ALCombatRules {
     ...
     * Protection bypass is based on {@linkplain AttributeRegistry#PROT_PIERCE Protection Pierce} and {@linkplain AttributeRegistry#PROT_SHRED Protection Shred}.
     * <p>
    ...
    public static float getDamageAfterProtection(LivingEntity target, DamageSource src, float amount, float protPoints) {
        if (src.getEntity() instanceof LivingEntity attacker) {
            float shred = (float) attacker.getAttributeValue(AttributeRegistry.PROT_SHRED.get());
            if (shred > 0.001F) {
                protPoints *= 1 - shred;
            }
            float pierce = (float) attacker.getAttributeValue(AttributeRegistry.PROT_PIERCE.get());
            if (pierce > 0.001F) {
                protPoints -= pierce;
            }
        }
     ...
     * Armor bypass is based on {@linkplain AttributeRegistry#ARMOR_PIERCE Armor Pierce} and {@linkplain AttributeRegistry#ARMOR_SHRED Armor Shred}.
     * <p>
    ...
    public static float getDamageAfterArmor(LivingEntity target, DamageSource src, float amount, float armor, float toughness) {
        if (src.getEntity() instanceof LivingEntity attacker) {
            float shred = (float) attacker.getAttributeValue(AttributeRegistry.ARMOR_SHRED.get());
            float bypassResist = Math.min(toughness * 0.02F, 0.6F);
            if (shred > 0.001F) {
                shred *= 1 - bypassResist;
                armor *= 1 - shred;
            }
            float pierce = (float) attacker.getAttributeValue(AttributeRegistry.ARMOR_PIERCE.get());
            if (pierce > 0.001F) {
